...
def get_data(vari,level):
    if level < 1:
        print("Error:No such variable exist ",vari)
        sys.exit()
...
def get_struct_data(s_name,vari,level):
    if level < 1:
        print("Error:No such Struct obj exist", vari)
        sys.exit()
        ...
        if vari not in variables[level][s_name][0].keys():
            print("Error:No varible" , vari, "exist in struct" ,s_name)
            sys.exit()
        return variables[level][s_name][0][vari][0]

def set_struct_data(s_name,vari,level,data):
    if level < 1:
        print("Error:No such Struct obj exist")
        sys.exit()
    elif s_name not in variables[level].keys():
        return get_struct_data(s_name,vari,level-1)
    else:
        ...
        if variables[level][s_name][0][vari][1] == 'INT' and type(data) != int:
            print("Error:Variable type MISMATCH", vari,data)
            sys.exit()
        ...
        if variables[level][s_name][0][vari][1] == 'DOUBLE' and type(data) != float:
            print("Error:Variable type MISMATCH", vari,data)
            sys.exit()
        if variables[level][s_name][0][vari][1] == 'CHAR' and (type(data) != str or len(data) > 1):
            print("Error:Variable type MISMATCH" , vari,data)
            sys.exit()
        variables[level][s_name][0][vari][0] = data

def check_struct(s_name,level):
    if level < 1:
        return 0
    elif s_name not in variables[level].keys():
        return check_struct(s_name,level-1)
    else:
        return 1
    ...
    if type(tree) == tuple:
        if tree[0] == '+':
            ...
            if tree[1] not in VARIABLE_TYPES:
                print("Error:Variable type Unknown")
                sys.exit()
                return "Error"
            elif tree[2][1] in variables[scope_level].keys():
                print("Error:Variable Already Declared")
                sys.exit()
                return "Error"
            val = run(tree[2][2])
            if tree[1] == 'INT' and type(val) != int:
                print("Error:Variable Type MISMATCH" , tree[2][1] , val)
                sys.exit()
                return "Error"
            ...
            if tree[1] == 'BOOL' and type(val) != bool:
                ...
                sys.exit()
                return "Error"
            if tree[1] == 'CHAR' and (type(val) != str or len(val) > 1):
                print("Error:Variable type MISMATCH", tree[2][1] , val)
                sys.exit()
            ...
            if typ == 'INT' and type(val) != int:
                print("Error:Variable Type MISMATCH" , tree[1] , val)
                sys.exit()
                return "Error"
            ...
            if typ == 'BOOL' and type(val) != bool:
                ...
                sys.exit()
                return "Error"
            if typ == 'CHAR' and (type(val) != str or len(val) > 1):
                print("Error:Variable type MISMATCH", tree[1] , val)
                sys.exit()
            ...
            if tree[1] == 'DEF':
                if tree[2] in structures.keys():
                    print("Error:Cannot redefine Struct " , tree[2])
                    sys.exit()
                structures[tree[2]] = {}
                for ele in tree[3]:
                    ...
                    if ele and ele[0] == 'INI':
                        print("Error:You cannot assign value in struct defination")
                        sys.exit()
            if tree[1] == 'INI':
                if tree[2] not in structures.keys():
                    print(tree[2] , 'Error: Variable type unknown')
                    sys.exit()
                if check_struct(tree[3],scope_level):
                    print(tree[3] , 'Error:Struct Variable already exist')
                    sys.exit()
                variables[scope_level][tree[3]] = {}
                variables[scope_level][tree[3]][0] = deepcopy(structures[tree[2]])
...
print("Output : ")
try:
    with open('test_cases/' +sys.argv[1], 'r') as file:
            s = file.read().replace('\n', '')
            # print(s)
except:
    print("File reading error:  Most probably file not found")
