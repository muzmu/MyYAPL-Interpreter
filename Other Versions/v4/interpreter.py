...
def get_data(vari,level):
    if level < 1:
        print("No such variable exist ",vari)
        sys.exit()
...
def get_struct_data(s_name,vari,level):
    if level < 1:
        print("No such Struct obj exist", vari)
        sys.exit()
...
def set_struct_data(s_name,vari,level,data):
    if level < 1:
        ...
        if variables[level][s_name][0][vari][1] == 'INT' and type(data) != int:
            print("Variable type MISMATCH", vari,data)
            sys.exit()
    ...
    if type(tree) == tuple:
        if tree[0] == '+':
            ...
            if tree[1] == 'INT' and type(val) != int:
                print("Variable Type MISMATCH" , tree[2][1] , val)
                sys.exit()
                return "Error"
            ...
            if typ == 'INT' and type(val) != int:
                print("Variable Type MISMATCH" , tree[1] , val)
                sys.exit()
                return "Error"
